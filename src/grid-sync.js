export default class GridSync {
   constructor(traces, axesLayout, gridlineCount) {
      this.traces = traces;
      this.axes = axesLayout;
    ...
    */
   sync() {
      const axisMetas = this.traces.map(trace => {
   ...
   getAxisMeta(axis) {
      ...
      const dtick = this._dtickCalculate(rangeDelta);
    ...
    */
   _dtickCalculate(range) {
      const rangeBase = this._bigFloor(range);
